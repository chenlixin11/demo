...
public class ValidateUtils {
	 ...
	 * @return 是整数返回true,否则返回false
	 */
	public static boolean isValidPositiveMoney(String str) {
		Pattern pattern = Pattern.compile("^\\d+(\\.\\d{1,2})?$");
		return pattern.matcher(str).matches();
	}
	...
	/**
	 * 
	 * @功能描述：校验入参是否是有效的时间格式（yyyyMMddHHmmss）
	 * @developer：庞皓
	 * @date：2017年9月11日 下午4:38:16
	 * @param timeStr
	 * @return
	 * boolean
	 */
	public static boolean checkValidTimeYyyyMMddHHmmss(String timeStr){  
	   if(ValidateUtils.isEmpty(timeStr) || timeStr.length() != 14 ){  
	       return false;
	   }  
	   try{  
	         int year = new Integer(timeStr.substring(0, 4)).intValue();  
	         int month = new Integer(timeStr.substring(4, 6)).intValue();  
	         int date = new Integer(timeStr.substring(6, 8)).intValue();
	         int hourOfDay = new Integer(timeStr.substring(8, 10)).intValue();
	         int minute = new Integer(timeStr.substring(10, 12)).intValue();
	         int second = new Integer(timeStr.substring(12, 14)).intValue();
	 ...
	 public static void main(String[] args) {
		System.out.println(checkValidTimeYyyyMMddHHmmss("20180101230059"));
	}
}
